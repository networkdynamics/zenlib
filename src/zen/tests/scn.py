...
class UndirectedSCNTestCase(unittest.TestCase):
	...
	def test_test2(self):
		G = scn.read(path.join(path.dirname(__file__),'test2.scn'),directed=False)
	...
	def test_test3(self):
		...
		G = scn.read(path.join(path.dirname(__file__),'test3.scn'), directed=False)
